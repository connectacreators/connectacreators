...
import { buildViralVideoCsv, downloadCsvFile } from "@/lib/viralVideoCsv";
...
export default function ViralToday() {
  ...
  const handleExportCsv = async () => {
    ...
    try {
      ...
      const csv = buildViralVideoCsv(rows, analysisById);
      const today = new Date().toISOString().slice(0, 10);
      downloadCsvFile(csv, `viral-today-export-${today}.csv`);
