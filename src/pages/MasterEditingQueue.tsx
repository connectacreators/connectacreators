...
export default function MasterEditingQueue() {
  ...
  useEffect(() => {
    ...
    if (status || postStatus || assignee) {
      console.warn("MasterEditingQueue: status/post_status/assignee filter params received but this page has no filter dropdowns yet");
      consumedAny = true;
    }
      ...
      <PageTransition className="flex-1 flex flex-col min-h-0 overflow-y-auto">
        ...
        <div className="flex-1 px-4 sm:px-8 py-8 max-w-6xl mx-auto w-full">
          ...
          {showArchive ? (
            ...
            <>
            <motion.div
              className="hidden lg:block rounded-xl border border-border/50 bg-card/30 overflow-hidden glass-card eq-table-wrap"
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.35, delay: 0.1 }}
            >
