...
// STATUS: user-selectable drawing palette — semantic per-pen choice, not theme surface
...
function SessionDropdown({ sessions, activeSessionId, onNewSession, onSwitchSession, onRenameSession, onDeleteSession }: {
  ...
  return (
    <div className="relative flex items-center gap-1" ref={ref}>
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute top-full left-0 mt-2 w-56 rounded-xl bg-[hsl(var(--cream))] border border-[hsl(var(--ink-on-cream))] shadow-xl z-50 overflow-hidden py-1">
          {sessions.length === 0 && (
...
function ToolsDropdown({ onAddNode }: { onAddNode: (type: "hookGeneratorNode" | "ctaBuilderNode" | "brandGuideNode" | "onboardingFormNode") => void }) {
  return (
    <div className="relative group">
      ...
      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-44 rounded-xl bg-[hsl(var(--cream))] border border-[hsl(var(--ink-on-cream))] shadow-xl z-50 overflow-hidden py-1 opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-opacity">
    ...
    <div className="absolute top-3 left-0 right-0 z-10 flex items-center justify-center pointer-events-none">
      ...
      {presenceOthers && myAnimalName && myColor && (
        <div className="absolute right-3 pointer-events-auto">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-2xl bg-[hsl(var(--cream))] border border-[hsl(var(--ink-on-cream))]">
            <PresenceAvatars
      ...
      <div className="pointer-events-auto flex flex-col items-center gap-1.5">
        {/* ── Main toolbar pill ── */}
        <div className="canvas-toolbar-pill flex items-center gap-0.5 px-2 py-1.5 rounded-2xl" style={{ background: "hsl(var(--cream))", border: "1px solid hsl(var(--ink-on-cream))", boxShadow: "2px 2px 0 hsl(var(--ink-on-cream))", color: "hsl(var(--ink-on-cream) / 0.65)" }}>
        ...
        {drawingMode && (
          <div className="flex items-center gap-0.5 px-2 py-1 rounded-2xl bg-[hsl(var(--cream))] border border-[hsl(var(--ink-on-cream))]">
            {!eraserMode && (
              <>
