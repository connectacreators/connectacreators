          ...
          >
            ...
            >
              ...
              <div className="relative group">
                <button className="p-1 rounded-lg hover:text-foreground hover:bg-muted/30 transition-colors" style={{ color: "hsl(var(--ink-on-cream) / 0.45)" }}>
              ...
              <button onClick={() => updateEdge({ arrow: !hasArrow })}
                className={`p-1 rounded-lg transition-colors ${hasArrow ? "text-[hsl(var(--aqua))] bg-[hsl(var(--aqua) / 0.15)]" : "hover:text-foreground hover:bg-muted/30"}`}
              style={!hasArrow ? { color: "hsl(var(--ink-on-cream) / 0.45)" } : undefined}
                  ...
                  <div className="flex gap-1.5">
                    {EDGE_COLORS.map(c => (
                      <button key={c} onClick={() => updateStyle({ color: c, stroke: c })}
                        className={`w-4 h-4 rounded-full border transition-transform hover:scale-110 ${edgeColor === c ? "border-[hsl(var(--ink-on-cream))] scale-110" : "border-black/10"}`}
                        style={{ background: c }}
                      />
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    {EDGE_WIDTHS.map(w => (
                      <button key={w} onClick={() => updateStyle({ width: w, strokeWidth: w })}
                        className={`w-6 h-6 rounded-lg flex items-center justify-center transition-colors ${edgeWidth === w ? "bg-[hsl(var(--aqua) / 0.2)]" : "hover:bg-muted/30"}`}
                      >
                        <div className="rounded-full" style={{ width: Math.max(3, w * 1.8), height: Math.max(3, w * 1.8), background: edgeWidth === w ? "hsl(var(--aqua))" : "hsl(var(--ink-on-cream) / 0.45)" }} />
                      </button>
                    ))}
                  </div>
                </div>
