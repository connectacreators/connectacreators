...
async function processChannel(
  ...
  const apifyRes = await fetch(
    `https://api.apify.com/v2/acts/${actorId}/runs?token=${APIFY_TOKEN}&waitForFinish=120`,
    {
      ...
      body: JSON.stringify(buildApifyInput(channel.platform ?? "instagram", cleanUsername, 200)),
    }
  );
  ...
  const videos = items
    .map((item: any) => {
      ...
      const videoId =
        item.shortCode ?? item.id ?? item.pk ?? item.videoId ?? item.aweme_id ?? null;

      const thumbnailUrl =
        item.displayUrl ?? item.thumbnailUrl ?? item.coverUrl ?? item.cover ?? item.previewUrl ?? item.thumbnail ?? null;
...
serve(async (req) => {
  ...
  try {
    ...
    for (let i = 0; i < channels.length; i += APIFY_MAX_CONCURRENT) {
      const batch = channels.slice(i, i + APIFY_MAX_CONCURRENT);
      console.log(`Processing batch ${Math.floor(i / APIFY_MAX_CONCURRENT) + 1}: ${batch.map(c => c.username).join(", ")}`);

      const batchResults = await Promise.all(
        batch.map((channel) => processChannel(supabase, channel))
      );
