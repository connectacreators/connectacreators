...
// Países objetivo. Vacío = alertar de TODOS los países. Agrega nacionalidades
// para filtrar (p.ej. "Venezuela", "Cuba", "México"), o define el secret
// IMMIGRATION_TARGET_COUNTRIES (separado por comas) para anular.
const TARGET_COUNTRIES: string[] = [
  // "Venezuela", "Cuba", "Haití", "Nicaragua", "México",
];

const RELEVANCE_THRESHOLD = 0.6; // 0..1 — quality over quantity
    ...
    try {
      ...
      for (const item of items.slice(0, 15)) {
        ...
        out.push({
          ...
          url: link,
          title,
          summary: pick("description").slice(0, 600),
...
Para cada noticia del arreglo, decide si es un DESARROLLO REAL Y RELEVANTE de inmigración que valga un video (cambios de política, TPS, asilo, parole, green card, cortes, USCIS, fechas límite, redadas, fallos judiciales). Descarta opinión genérica, clickbait, o noticias no migratorias.
...
- "score": qué tan valiosa es para un video (0=nada, 1=urgente/altísima).
...
function targetCountries(): string[] {
  const env = Deno.env.get("IMMIGRATION_TARGET_COUNTRIES");
  const list = env ? env.split(",") : TARGET_COUNTRIES;
...
function alertHtml(row: {
  ...
  const badges = (row.countries || []).map((c) =>
    `<span style="display:inline-block;background:#E6C780;color:#2A2008;font-weight:700;font-size:12px;padding:4px 10px;border-radius:99px;margin:0 6px 6px 0">${c}</span>`
  ).join("");
  ...
  <div style="padding:26px">
    ...
    <h1 style="font-size:22px;line-height:1.25;margin:0 0 12px">${row.title}</h1>
    <p style="font-size:15px;line-height:1.6;color:rgba(244,240,232,0.72);margin:0 0 18px">${(row.summary || "").slice(0, 400)}</p>
    <p style="font-size:13px;color:rgba(244,240,232,0.5);margin:0 0 4px"><b style="color:#E6C780">Por qué importa:</b> ${row.reason || "—"}</p>
...
async function sendAlert(row: Parameters<typeof alertHtml>[0]) {
  ...
  const flag = (row.countries || [])[0] && !/general/i.test(row.countries[0]) ? `${row.countries[0]} · ` : "";
  await client.send({
    from: smtpUser,
    to: recipients(),
    subject: `🚨 ${flag}Noticia de inmigración: ${row.title.slice(0, 90)}`,
    content: `${row.title}\n\n${row.summary}\n\nGenerar ángulo de video: ${FUNCTIONS_BASE}/immigration-video-angle?id=${row.id}&t=${ANGLE_TOKEN}\n\nFuente: ${row.url}`,
    html: alertHtml(row),
  });
  await client.close();
  return true;
...
Deno.serve(async (req) => {
  ...
  try {
    // 1. gather + cap
    const raw = [...(await fetchFederalRegister()), ...(await fetchGoogleNews())].slice(0, MAX_CANDIDATES);
    ...
    const rows = fresh.map((c, i) => {
      ...
      const relevant = !!s?.relevant && (s?.score ?? 0) >= RELEVANCE_THRESHOLD;
      return {
