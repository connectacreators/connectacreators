...
async function handleSMSStep(
  ...
  try {
    ...
    if (accountSid && authToken && from) {
      ...
      // Log SMS for testing (Twilio not configured)
...
async function handleWhatsAppStep(
  ...
  try {
    ...
    // If Twilio credentials are configured, send real WhatsApp message
    if (accountSid && authToken && from) {
      try {
        const res = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${accountSid}/Messages.json`, {
          method: 'POST',
        ...
        if (!res.ok) {
          ...
          return {
            ...
            action: 'send_whatsapp',
            status: 'failed',
            error: `Twilio WhatsApp API error: ${error}`,
            duration: Date.now() - startTime,
          };
        }
      ...
      // Log WhatsApp for testing (Twilio not configured)
