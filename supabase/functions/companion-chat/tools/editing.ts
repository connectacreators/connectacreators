...
export const EDITING_TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    description: "Control the in-page review panel that's CURRENTLY OPEN on the Command Deck (see ACTIVE ACTION SURFACE in context — only usable when one is open; if none is open, use open_editing_item first). Play/pause/seek the video, add a timestamped revision note, mark all outstanding revisions resolved ('approve'), download the video, or close the panel entirely. Targets the open item directly — do not pass client_name or item_title.",
    input_schema: {
      type: "object",
      properties: {
        action: { type: "string", enum: ["play", "pause", "seek_to", "add_note", "resolve_all", "download", "close"], description: "play/pause the video, seek_to a timestamp, add_note (a revision comment), resolve_all (approve — marks every outstanding revision note resolved), download (saves the video file), or close (dismiss the panel, e.g. 'close this'/'close the tab', returning to the orb)." },
...
export async function handleEditingTool(
  ...
  if (block.name === "control_review_surface") {
    ...
    const validActions = ["play", "pause", "seek_to", "add_note", "resolve_all", "download", "close"];
